...
function employeeUpdater () {
  for (let p in employees){
    if(employees[p].firstName==='Theo'){
      delete employees[p];
    }
    else if(employees[p].firstName==='Lorie'){
      employees[p].department='HR';
    }
  }
  return employees;
}
...
function removeDuplicates () {
  return workplaceAccidents.filter((val, ind, arr)=>ind==arr.indexOf(val), []); 
}
